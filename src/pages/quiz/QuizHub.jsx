    ...
    <div style={{
      ...
      <style>{`
        @media (max-width: 768px) {
          .quiz-hub-header h1 { font-size: 2rem !important; }
          .quiz-hub-header p { font-size: 1rem !important; }
          .quiz-hub-actions { flex-direction: column !important; width: 100% !important; }
          .quiz-hub-actions button { width: 100% !important; }
          .quiz-card-header { flex-direction: column !important; gap: 12px !important; }
          .quiz-card-controls { width: 100% !important; justify-content: flex-start !important; }
        }
      `}</style>
      <div style={{ maxWidth: 1000, margin: '0 auto' }}>
        <header className="quiz-hub-header" style={{ textAlign: 'center', marginBottom: 'clamp(30px, 8vw, 60px)' }} data-aos="zoom-in">
           <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 15, marginBottom: 20 }}>
              <div style={{ width: 'clamp(40px, 8vw, 50px)', height: 'clamp(40px, 8vw, 50px)', background: 'linear-gradient(135deg, #3b82f6, #8b5cf6)', borderRadius: 12, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                 <i className="pi pi-bolt" style={{ fontSize: 'clamp(1.2rem, 3vw, 1.5rem)', color: '#fff' }}></i>
              </div>
              <h1 style={{ fontSize: 'clamp(1.8rem, 5vw, 2.5rem)', fontWeight: 900, letterSpacing: '-0.02em', fontFamily: 'Outfit' }}>Assessment Center</h1>
           </div>
           <p style={{ color: '#94a3b8', fontSize: 'clamp(1rem, 2.5vw, 1.2rem)', maxWidth: 600, margin: '0 auto', lineHeight: 1.5 }}>Test your knowledge or create custom assessments for your team.</p>
           
           {!user ? (
             <div style={{ marginTop: 40 }}>
                <Button label="Identify Yourself to Continue" icon="pi pi-user" onClick={() => setShowLogin(true)} 
                        style={{ padding: '15px 30px', borderRadius: 50, background: '#3b82f6', border: 'none', fontWeight: 700, width: '100%', maxWidth: 350 }} />
             </div>
           ) : (
             <div className="quiz-hub-actions" style={{ marginTop: 40, display: 'flex', justifyContent: 'center', gap: 15 }}>
        ...
        <section>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 30 }}>
            <h2 style={{ fontSize: 'clamp(1.4rem, 4vw, 1.8rem)', fontWeight: 800 }}>Available Quizzes</h2>
            <div style={{ width: 1, height: 2, background: 'rgba(255,255,255,0.1)', flex: 1, margin: '0 20px' }}></div>
          </div>

          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(clamp(280px, 45%, 350px), 1fr))', gap: 25 }}>
            {quizzes.map((quiz, i) => (
              <Card key={quiz.id} data-aos="fade-up" data-aos-delay={i * 100} style={{ 
                ...
                transition: 'transform 0.3s ease, border 0.3s ease'
                 ...
                 <div style={{ padding: 'clamp(16px, 4vw, 25px)' }}>
                    <div className="quiz-card-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 15 }}>
                       <h3 style={{ fontSize: 'clamp(1.2rem, 3vw, 1.4rem)', fontWeight: 800, margin: 0, color: '#fff' }}>{quiz.title}</h3>
                       <div className="quiz-card-controls" style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
                       ...
                       <Button label="Take Quiz" icon="pi pi-play" 
                               ...
                               style={{ borderRadius: 12, padding: '10px 18px', background: '#3b82f6', border: 'none', fontWeight: 700 }} />
                    </div>
                 </div>
              </Card>
            ))}
      ...
      <Dialog header="Join the Orbit" visible={showLogin} style={{ width: '90vw', maxWidth: 400 }} onHide={() => setShowLogin(false)} closable={false}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 20, padding: '20px 0' }}>
            <div className="flex flex-column gap-2">
              <label htmlFor="name" style={{ fontWeight: 600, color: '#334155' }}>Display Name</label>
              <InputText id="name" value={name} onChange={(e) => setName(e.target.value)} placeholder="How should we call you?" style={{ borderRadius: 10 }} />
            </div>
            <div className="flex flex-column gap-2">
              <label htmlFor="email" style={{ fontWeight: 600, color: '#334155' }}>Email Address</label>
              <InputText id="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="your@email.com" style={{ borderRadius: 10 }} />
            </div>
            <Button label="Enter Orbit" onClick={handleRegister} className="mt-2" 
                    style={{ background: 'linear-gradient(to right, #3b82f6, #8b5cf6)', border: 'none', borderRadius: 12, padding: 12, fontWeight: 700 }} />
          </div>
      </Dialog>
    </div>
  );
